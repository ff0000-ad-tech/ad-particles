 ...
 * </codeblock>
 */
import { Vector2D, SimpleNoise2D } from "@ff0000-ad-tech/ad-geom";
import ParticleModel from "./ParticleModel";
import ParticlesUtils from "./ParticlesUtils";
...
var Particle = function(ctx, model, fps) {
  ...
  self._lifeSpanFrameCount = ParticlesUtils.secToFrameCount(
    self.properties.lifeSpan,
    self.fps
  );
...
Particle.prototype = {
  ...
  update: function() {
    ...
    if (this._hasTurbulence) {
      if (!this.noiseStart) {
        this.noiseStart = new Vector2D();
        this.noiseStart.random();
